...
tts_models = {
    ...
    "Indian": "tts_models/en/ljspeech/tacotron2-DDC",
    "British": "tts_models/en/ek1/tacotron2",
}

# Initialize TTS with default voice
current_tts_model = "Jenny"
tts = TTS(model_name=tts_models[current_tts_model], progress_bar=False).to(device)
...
class Agent:
    def __init__(self, name, system_prompt, tts_model="Jenny", template_image=None):
        self.name = name
...
def update_tts_model():
    ...
    if current_agent:
        tts = TTS(model_name=tts_models[current_agent.tts_model], progress_bar=False).to(device)
...
def apply_config(config):
    ...
    if config:
        ...
        current_tts_model = config.get('TTS', 'default_voice', fallback="Jenny")
...
strength_var = tk.DoubleVar(value=0.3)
...
# Header
header_frame = ttk.Frame(left_frame, padding="10", style="TFrame")
header_frame.pack(fill=tk.X)

title_label = ttk.Label(header_frame, text="AI Chatbot", font=("Helvetica", 24, "bold"))
title_label.pack(side=tk.LEFT)

# Load and display an icon (replace 'icon.png' with your icon file)
...
# Conversation
conversation_frame = ttk.Frame(left_frame, padding="10", style="TFrame")
...
# Image display frame
image_frame = ttk.LabelFrame(left_frame, text="Generated Image", padding="10", style="TLabelframe")
...
voice_var = tk.StringVar(value="Jenny")
...
toggle_image_gen_button = ttk.Button(button_frame, text="ImageGen", command=toggle_image_generation, width=10)
...
#exit_button.grid(row=3, column=3, padx=5, pady=10, sticky="e")
...
volume_meter_label = ttk.Label(audio_params_frame, text="Volume Meter:")
...
# Image parameters
image_params_frame = ttk.LabelFrame(params_frame, text="Image Parameters", padding="10", style="TLabelframe")
